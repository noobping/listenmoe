...
const HOP: usize = 512;
const BAR_RISE_BLEND: f32 = 0.32;
const BAR_FALL_BLEND: f32 = 0.18;
...
fn process_samples_for_viz_inner(
    ...
    while fft_state.mono_ring.len() >= FFT_SIZE {
        ...
        for i in 0..fft_state.bars.len() {
            let current = fft_state.bars_smooth[i];
            let target = fft_state.bars[i];
            let blend = if target > current {
                BAR_RISE_BLEND
            } else {
                BAR_FALL_BLEND
            };
            fft_state.bars_smooth[i] = current + (target - current) * blend;
        }
